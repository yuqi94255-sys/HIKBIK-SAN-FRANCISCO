...
/// 真實示範：僅保留 Jessica Martinez「Angels Landing Summit」；點擊卡片 NavigationLink 全屏進入詳情。
            ...
            if let onSave = onSaveTap {
                Button(action: onSave) {
                    Image(systemName: isSaved ? "bookmark.fill" : "bookmark")
                        .font(.system(size: 16))
                        .foregroundStyle(isSaved ? CommunityColors.accentGreen : CommunityColors.textMuted)
                }
                .buttonStyle(.plain)
            } else {
