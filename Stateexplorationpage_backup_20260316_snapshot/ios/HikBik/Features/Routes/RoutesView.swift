...
// MARK: - Route colors (dark theme, 深海藍 #0B121F)，internal 供 RouteFavoritesView 等共用
enum RouteColors {
    static let background = Color(hex: "0B121F")
    static let cardBg = Color(hex: "2A3540")
    static let textPrimary = Color.white
    static let textMuted = Color(hex: "9CA3AF")
    /// Search / input placeholder only (brighter for contrast on dark backgrounds).
...
struct RoutesView: View {
    // MARK: - Mock data（static 供 RouteFavoritesView 解析收藏用，與 RIDB 零關係）
    private static let trendingRoutes: [RouteItem] = [
    ...
    private static let proGuideRoutes: [RouteItem] = [
    ...
    private static let nationalParkForestRoutes: [RouteItem] = [
    ...
    private static let familyRoutes: [RouteItem] = [
    ...
    private static let officialHikingRoutes: [RouteItem] = [
    ...
    private static let officialBikingRoutes: [RouteItem] = [
    ...
    private var allRoutesContent: some View {
        VStack(alignment: .leading, spacing: 0) {
            routeSection(title: "Trending Now", icon: "chart.line.uptrend.xyaxis", routes: Self.trendingRoutes)
            routeSection(title: "Pro Guide Routes", icon: "person.badge.shield.checkmark.fill", routes: Self.proGuideRoutes)
            routeSection(title: "National Park National Forest", icon: "leaf.fill", routes: Self.nationalParkForestRoutes)
            routeSection(title: "Family Route", icon: "tent.fill", routes: Self.familyRoutes)
            Spacer(minLength: 80)
        }
    ...
    private var aiPlannerContent: some View {
        VStack(alignment: .leading, spacing: 20) {
            ...
            ZStack(alignment: .leading) {
                if aiPlannerInput.isEmpty {
                    Text("e.g. 3-day hike in Yosemite with lakes")
                        .font(.system(size: 16))
                        .foregroundStyle(RouteColors.searchPlaceholder)
                        .padding(16)
                }
                TextField("", text: $aiPlannerInput)
                    .font(.system(size: 16))
                    .foregroundStyle(RouteColors.textPrimary)
                    .padding(16)
            }
    ...
    private var officialRoutesContent: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            routeSection(title: "Official Macro Journeys", icon: "mountain.2.fill", routes: Self.trendingRoutes)
            routeSection(title: "Expert Micro Tracks", icon: "scope", routes: Self.proGuideRoutes)
            routeSection(title: "Family & Beginner", icon: "person.3.fill", routes: Self.familyRoutes)
            routeSection(title: "Official Hiking", icon: "figure.hiking", routes: Self.officialHikingRoutes)
            routeSection(title: "Official Biking", icon: "bicycle", routes: Self.officialBikingRoutes)
            Spacer(minLength: 80)
        }
        .animation(.easeInOut(duration: 0.25), value: activeTab)
    }

    /// Official Detailed Track 入口：Half Dome Cables，點擊進入官方詳情頁
                        ...
                        ZStack(alignment: .topTrailing) {
                            ...
                            Button {
                                ...
                                Image(systemName: routeFavorites.isFavorite(route.id) ? "heart.fill" : "heart")
                                    .font(.system(size: 18))
                                    .foregroundStyle(routeFavorites.isFavorite(route.id) ? .red : RouteColors.textMuted)
                                    .padding(10)
    ...
    /// 官方路線全集（與 RIDB 零關係），供 RouteFavoritesView 依 id 解析跳轉
    static var allRouteItems: [RouteItem] {
        trendingRoutes + proGuideRoutes + nationalParkForestRoutes + familyRoutes + officialHikingRoutes + officialBikingRoutes
    }
}

// MARK: - Yosemite Half Dome 入口卡（Official Detailed，戰術紫 OFFICIAL，跳轉官方詳情頁）
