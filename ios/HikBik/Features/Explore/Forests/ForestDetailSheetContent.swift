...
struct ForestDetailSheetContent: View {
    ...
    var body: some View {
        ...
        .overlay(alignment: .topTrailing) {
            FavoriteButton(
                id: "nationalforest:\(forest.id)",
                name: forest.name,
                category: .forest,
                agency: "U.S. Forest Service",
                imageUrl: forest.photos?.first,
                latitude: effectiveCoordinate?.latitude ?? 0,
                longitude: effectiveCoordinate?.longitude ?? 0,
...
struct FacilityDetailSheet: View {
    ...
    var body: some View {
        ...
        .overlay(alignment: .topTrailing) {
            FavoriteButton(
                id: "nationalforest:\(forestId):facility:\(facility.id)",
                name: facility.name,
                category: .forest,
                agency: "U.S. Forest Service",
                imageUrl: nil,
                latitude: directionsCoordinate?.latitude ?? 0,
                longitude: directionsCoordinate?.longitude ?? 0,
                isFavorite: $isFavorite
            )
            .padding(.top, 52)
            .padding(.trailing, 20)
        }
        .onAppear { isFavorite = FavoritesManager.shared.contains(id: "nationalforest:\(forestId):facility:\(facility.id)") }
    }
